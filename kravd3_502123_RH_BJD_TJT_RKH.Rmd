...
ggplot(p_samadag, aes(x = n_lesandi, y = p_sdag)) +
  geom_point() +
  geom_vline(xintercept = qbirthday(prob = 0.5, classes = 365, coincident = 2), linetype = "dashed") +
  geom_vline(xintercept = qbirthday(prob = 0.7, classes = 365, coincident = 2), linetype = "dashed") +
  geom_vline(xintercept = qbirthday(prob = 0.9, classes = 365, coincident = 2), linetype = "dashed") +
  xlab("Antal lesandi") +
  ylab("Sannlíkindi fyri sama føðingardag")
...
> 2f3) um tú hyggur eftir virðunum `50, 100, 200, 300, 400` fyri antal lesandi, hvørji sannlíkindi `p_sdag` fyri `max == 2, 3, 4, 5` eru ávikavist hægst, næst- og trið-hægst í hvørjum føri? (tip: avles plottið í 2f2).
    ...
    Við avlesing av grafinum, sæst greitt at tá x = 400, eru sannlíkjindini fyri == 5 størst, == 4 næst størst, aftaná hetta ber ikki til at siga hvat kemur bert við at avlesa grafin.
...
ggplot(ferdslu_ohapp) +
  ...
  geom_boxplot(aes(x = `Skadd fólk` / `Ferðsluóhapp tilsamans`, y = `sýsla`), color = "blue", na.rm = TRUE) +
  geom_violin(aes(x = `Skadd fólk` / `Ferðsluóhapp tilsamans`, y = `sýsla`), alpha = 0.25, na.rm = TRUE)
```
...
```{r}
`stað_1` <- ophapp_yvirlit %>% filter(`stað` == "Á kommunalvegi")
prop.test(x = 10, n = `stað_1`$n, p = `stað_1`$p_skadd)

`stað_2` <- ophapp_yvirlit %>% filter(`stað` == "Á landsvegi")
prop.test(x = 10, n = `stað_2`$n, p = `stað_2`$p_skadd)
```
