...
```
    ...
    Um ein bert hyggur eftir plottinum, so skal tað vera ~22 ella 23 fyri 50%, ~30 fyri 70% og ~41 fyri 90%.
...
ggplot(p_samadag, aes(x = n_lesandi, y = p_sdag)) +
  geom_point() +
  geom_hline(yintercept = qbirthday(prob = 0.5, classes = 365, coincident = 2) / 100) +
  geom_hline(yintercept = qbirthday(prob = 0.7, classes = 365, coincident = 2) / 100) +
  geom_hline(yintercept = qbirthday(prob = 0.9, classes = 365, coincident = 2) / 100) +
  xlab("Antal lesandi") +
  ylab("Sannlíkindi fyri sama føðingardag")
